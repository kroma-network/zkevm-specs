...
def begin_deposit_tx(instruction: Instruction):
    ...
    tx_mint = instruction.tx_context_lookup(tx_id, TxContextFieldTag.Mint)

    # NOTE(chokobole): Mint ETH, which is required by kroma-zkevm.
    tx_mint_rlc = instruction.rlc_encode(tx_mint, 32)
    instruction.add_balance(tx_caller_address, [tx_mint_rlc], None)

    # CallerAddress != 0 (not a padding tx)
    instruction.constrain_not_zero(tx_caller_address)

    # NOTE(chokobole): For deposit tx, it doesn't verify nonce.
    ...
    invalid_tx = 1 - (1 - balance_not_enough) * (1 - gas_not_enough)
